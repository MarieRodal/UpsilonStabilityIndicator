# -*- coding: utf-8 -*-
"""
Created on Thu Dec  8 15:54:47 2022


@author: Marie Rodal
"""

#Uses AR00 as a base class
...
class multiResultClass():
    ...
    def __init__(self, index, x_tau, Y , BIC10, BICpq, order, persistence):
        self.index = index
        self.x_tau = x_tau
        self.Y = Y
        self.BIC00 = BIC10
...
def Y_ARIMA_i(time_series, tau, p_max, q_max, i,wind_shift):
    ...
    results = multiResultClass(None, None, None, None, None, None, None)
    #save i
    results.index = i
    ...
    #calculate BIC for AR(0)
    
    try:
        d = results.order[1]  
        order10 = robjects.FloatVector([0,d,0])
        AR0 = forecast.Arima(resultsr, order=order10)
    except:
        None
    ...
    try:
        results.Y = 1 - np.exp(-abs(results.BICpq-results.BIC10)/tau)
